...
def main(args: argparse.Namespace) -> None:
    ...
    if not os.path.isdir(args.input):
        ...
        studydirs = [f for f in os.listdir(args.input) if os.path.isdir(f)]

    for s in studydirs:
        spath = os.path.join(args.input,s,'sam')
        files = os.listdir(spath)
        images = [ f for f in files if f.startswith('slice') ]
        ...
        for m,t in zip(masks,images):
            ...
            mask,_ = loadnifti(m)
            if mask is None:
                print('coule not load {} as a mask, skipping...'.format(m))
                continue

            batched_input.append({
                    'image': prepare_image(image, resize_transform, sam),
                    'mask_inputs': prepare_image(mask, resize_transform, sam),
